...
(packet-in)される。この動作はこのあとの OpenFlow バージョンでは変更され
ているため明示的にルールを指定する。別途記載するが、Trema にのパケットパー
サ(Trema/Pio)で特定のパケットがただしくパースできず、コントローラ停止す
るバグがあったため、NetTesterでは default DROP を採用している
\footnote{default CONTROLLER はデバッグ用途で使用することができる。}。
% TODO: 後述のところうめる。
