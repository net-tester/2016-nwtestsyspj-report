...
いる。Phutの使いかたやNetTesterでの応用については付
録~\ref{sec:phut_basics}を参照のこと。
...
あった\footnote{修正済。デバッグについての補足情報については付
録~\ref{sec:debugging-trema}を参照。}。そのため、NetTesterでは default
DROP を採用している。
...
\begin{itemize}
 ...
 \item OpenFlow Message (PortMod) を物理OpenFlowスイッチに送信し、ポート
       状態変更の操作をおこなう。
\end{itemize}
今回、NetTester では、物理OpenFlowスイッチにExpectベースのツール(付
録~\ref{sec:expectacle})でログイン/コマンド実行し、直接スイッチ上でポート
