...
\begin{itemize}
 ...
 \item NetTesterはバックエンドで\code{ip}コマンドを使用してNetwork
       ...
       しているTrema/Phutが\code{ip}コマンドを使用している。付
       録~\ref{sec:phut_basics}参照。}。このときroot権限が必要になり
...
NetTesterが設定するため、事前設定は不要である。NetTesterを起動すると、
\ref{sec:nettester-model-restriction}節に示したDPIDでOpen vSwitchのブリッ
ジが作成され、OFCとの接続設定がおこなわれる。
...
正しく通信がおこなえない問題がおきた(付
録~\ref{sec:tcp-checksum-offload-trouble})。ワークアラウンドとして、
...
この他に、テスト対象ネットワーク機器操作のために使うツール(付
録~\ref{sec:expectacle})でも、テストシナリオ実行の際に環境変数を使用する
ので注意すること。
