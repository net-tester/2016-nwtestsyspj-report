...
\begin{itemize}
 \item \tabref{tab:nettester-software-stack}以外に必要な一般的な開発ツー
       ル等については Build Essential\footnote{\code{sudo apt install
       build-essential} ,
...
\tabref{tab:tools-for-scenario}に示すツールを使用している。現時点
 ...
 \item NetTesterを終了する
       \begin{itemize}
        ...
        \item NetTesterが生成したSSW, テスト用ノードなどを削除する。テス
              ...
              namespace であるため、OS上に設定が残る。適切な終了(削除)処
              理がおこなわれない場合、OS上にこれらの設定が残りつづけてし
              まうため注意が必要である。
       \end{itemize}
\end{enumerate}
